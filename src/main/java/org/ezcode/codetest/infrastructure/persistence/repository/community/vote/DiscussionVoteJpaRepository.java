...
public interface DiscussionVoteJpaRepository extends JpaRepository<DiscussionVote, Long> {
	...
	@Query("SELECT COUNT(dv) FROM DiscussionVote dv WHERE dv.discussion.id = :discussionId AND dv.voteType = :voteType")
	Long countUpvoteByDiscussionId(Long discussionId, VoteType voteType);

	@Query("SELECT COUNT(dv) FROM DiscussionVote dv WHERE dv.discussion.id = :discussionId AND dv.voteType = :voteType")
	Long countDownvoteByDiscussionId(Long discussionId, VoteType voteType);
