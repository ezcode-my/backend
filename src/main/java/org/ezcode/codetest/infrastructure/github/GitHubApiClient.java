...
public class GitHubApiClient {
    ...
    protected Optional<String> fetchSourceBlobSha(GitHubPushRequest req) {
        ...
        String path = String.format("ezcode/%s/%s", req.difficulty(), fileName);
    ...
    protected String createTree(
        ...
        JsonNode treeRes = baseBuilder(req.accessToken())
            .post()
            .uri("/repos/{owner}/{repo}/git/trees", req.owner(), req.repo())
    ...
    private String createCommit(GitHubPushRequest req, String parentSha, String treeSha) {
        ...
        JsonNode commitRes = baseBuilder(req.accessToken())
            .post()
            .uri("/repos/{owner}/{repo}/git/commits", req.owner(), req.repo())
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(body)
            .retrieve()
            .onStatus(HttpStatusCode::is4xxClientError,
    ...
    private WebClient baseBuilder(String token) {
        String accessToken;
        try {
            accessToken = aesUtil.decrypt(token);
        } catch (Exception e) {
            throw new GitHubClientException(GitHubExceptionCode.INVALID_ACCESS_TOKEN);
        }
